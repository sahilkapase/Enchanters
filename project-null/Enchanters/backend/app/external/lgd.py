...
STATE_DISTRICTS: dict[str, list[str]] = {
    ...
    "Assam": ["Baksa", "Barpeta", "Cachar", "Darrang", "Dhubri", "Dibrugarh", "Golaghat", "Jorhat", "Kamrup", "Kamrup Metropolitan", "Karbi Anglong", "Kokrajhar", "Lakhimpur", "Nagaon", "Nalbari", "Sivasagar", "Sonitpur", "Tinsukia"],
    ...
    "Gujarat": ["Ahmedabad", "Amreli", "Anand", "Banaskantha", "Bharuch", "Bhavnagar", "Dahod", "Gandhinagar", "Junagadh", "Kutch", "Mehsana", "Narmada", "Navsari", "Panchmahal", "Patan", "Porbandar", "Rajkot", "Sabarkantha", "Surat", "Surendranagar", "Tapi", "Vadodara", "Valsad"],
...
async def get_districts_from_api(state: str) -> list[str]:
    """Try LGD API for districts, fall back to local data."""
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            response = await client.get(
                f"{settings.LGD_API_URL}/districts",
                params={"state": state},
            )
            if response.status_code == 200:
                data = response.json()
                return [d.get("name", "") for d in data.get("districts", [])]
    except Exception as e:
        logger.warning("LGD API failed for state %s: %s", state, str(e))
